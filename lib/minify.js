...
async function minify(files, options) {
    ...
    if (options.mangle) {
        if (options.deadcode_inject) {
            if (!Array.isArray(options.deadcode_inject.dead_codes)) {
                options.deadcode_inject.dead_codes = [];
            }
            
            options.deadcode_inject.dead_codes = options.deadcode_inject.dead_codes.concat([
                "window._$PINO && (p.p1 = new Date());",
                "window._$PINO && (p.p2 = new Date());",
                "window._$PINO && (p.p3 = new Date());",
                "window._$PINO && (p.p4 = new Date());",
                "window._$PINO && (p.p5 = new Date());",
            ]);

            if (typeof options.deadcode_inject.ratio !== typeof 0) {
                options.deadcode_inject.ratio = 0.2;
            }

            if (typeof options.deadcode_inject.random_seed !== typeof "") {
                options.deadcode_inject.random_seed = "";
            }

            options.mangle.deadcode_inject = options.deadcode_inject;
        }
    ...
    if (options.mangle) {
        ...
        // PI_BEGIN
        if (options.shuffle_mangle) {
            base54.setShuffleSeed(options.shuffle_mangle);
        }
        // PI_END
